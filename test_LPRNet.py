    ...
    for i in range(epoch_size):
        ...
        for i in range(prebs.shape[0]):
            ...
            for j in range(preb.shape[1]):
                preb_label.append(np.ma.argmax(preb[:, j], axis=0))
...
def show(img, label, target):
    '''
    show the test result of each test image
    '''
...
def cv2ImgAddText(img, text, pos, textColor=(255, 0, 0), textSize=12):
    '''
    plot the result text in test image
    '''
    if (isinstance(img, np.ndarray)):  # detect opencv format or not
        img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    draw = ImageDraw.Draw(img)
    fontText = ImageFont.truetype("weights/NotoSansCJK-Regular.ttc", textSize, encoding="utf-8")
