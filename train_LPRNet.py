...
def train():
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if (iteration + 1) % args.interval == 0:
            print('Epoch:' + repr(epoch) + ' || epochiter: ' + repr(iteration % epoch_size) + '/' + repr(epoch_size)
                  ...
                  'Batch time: %.4f sec || ' % (end_time - start_time)+'Train Accuracy: %.4f'%(Acc))
    ...
    plt.plot(Test_Accuracy, color='red', label='testing accuracy')
    plt.xlabel('Detection Width')
    plt.ylabel('Accuracy[%]')
    plt.subplot(122)
    ...
    torch.save(lprnet.state_dict(), args.save_folder + 'LPRNet_model.pth')

def Greedy_Decode_Eval(Net, datasets, args):
    '''
    Get the test result with greedy decode
    '''
